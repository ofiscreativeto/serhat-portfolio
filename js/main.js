function buildGrid(projects) {
  ...
  projects.forEach(function(p) {
    ...
    a.innerHTML = '<div class="thumb"><img src="' + (p.thumbnail || '') + '" alt="" loading="lazy" data-vimeo-thumb="' + p.vimeoId + '"><div class="play-overlay"><span class="overlay-title">' + (p.title || '') + '</span></div></div>';
    reel.appendChild(a);
  });
...
function initModal() {
  ...
  document.querySelectorAll('.frame').forEach(function(f) {
    f.addEventListener('click', function(e) {
      e.preventDefault();
      iframe.src = 'https://player.vimeo.com/video/' + this.dataset.vimeoId + '?autoplay=1';
